...
import {
  useBuyerQuery,
  useBuyerUpdateMutation,
} from "@/redux/features/buyer/buyerApi";
import { useSession } from "next-auth/react";
import React from "react";
import toast from "react-hot-toast";

type IEditBuyerPageProps = {
  params: any;
};

const EditBuyerPage = ({ params }: IEditBuyerPageProps) => {
  ...
  const { data, isLoading, isError } = useBuyerQuery(id);
  const [updateBuyer] = useBuyerUpdateMutation();
  ...
  const onSubmit = async (values: any) => {
    try {
      ...
      await updateBuyer({ formData, id }).unwrap();
  ...
  return (
    <section className="p-5">
      ...
      <h1 className="my-5 text-2xl font-semibold">Update Buyer Information</h1>
